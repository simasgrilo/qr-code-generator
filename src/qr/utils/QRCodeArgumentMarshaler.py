...
class QRCodeArgumentMarshaler:
    ...
    def process_args(cmd_input: List[str]):
        ...
        """
        if len(cmd_input) != 2 and len(cmd_input) != 4:
    ...
    def generate_qr_code(cmd_input: List[str]):
        ...
        """
        marshaled_args = QRCodeArgumentMarshaler.process_args(cmd_input)
        data, path, error_correction_level, version = None, None, None, None
        if len(marshaled_args) == 4:
            data, path, error_correction_level, version = marshaled_args
        else:
            data, path = marshaled_args
            version, error_correction_level = QRCodeFactory.determine_min_qr_code_size(data)
